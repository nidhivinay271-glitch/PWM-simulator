...
try:
    ...
    raise ImportError(
        "Streamlit is not installed. Install it using: pip install streamlit"
    )
...
try:
    ...
    # Import pyplot after setting backend so plotting works in Streamlit
...
# Global maximum voltage used to scale signals (default 5V)
VMAX = 5.0
# === NEW FEATURE START ===
...
def simulate_rc_response(time_array_ms, input_signal, resistance_ohm, capacitance_f):
    ...
    for i in range(1, len(input_signal)):
        ...
        vc[i] = vc[i - 1] + alpha * (input_signal[i] - vc[i - 1])
    return vc
...
def simulate_rl_response(time_array_ms, vin, inductance_h, resistance_ohm):
    ...
    for i in range(1, len(vin)):
        ...
        # steady-state current for this sample (vin assumed constant over dt)
        i_inf = vin[i] / resistance_ohm
        current[i] = current[i - 1] * exp_factor + i_inf * (1.0 - exp_factor)
        current[i] = max(current[i], 0.0)
    v_r = current * resistance_ohm
    di_dt = np.gradient(current, dt_s)
    v_l = inductance_h * di_dt
...
def get_device_response(device, time_array_ms, pwm_signal, params):
    ...
    if device == "LED":
        ...
        # Simplified, physically-consistent LED model:
        # - If VIN <= Vf -> I = 0
        # - Otherwise I = (VIN - Vf) / R_series (Ohm's law with fixed forward drop)
        led_current = np.zeros_like(vin, dtype=float)
        above = vin > vf
        led_current[above] = (vin[above] - vf) / r_led
        # clip to a safe max (50 mA)
        led_current = np.clip(led_current, 0.0, 0.05)

        # smoothing (perceived brightness)
    ...
    if device == "Motor":
        ...
        for i in range(1, len(vin_scaled)):
            ...
            effective_v = np.clip(vin_scaled[i - 1] - back_emf, -motor_supply_v, motor_supply_v)
    ...
    if device == "Heater":
        ...
        ambient_temp = 25.0
        # Thermal resistance (R_th) and thermal capacitance (C_th) produce a time constant tau_th = R_th * C_th
        r_th = 10.0
        tau_th = max(1e-6, float(params["heater_inertia_ms"]) / 1000.0)
        c_th = tau_th / r_th
...
if selected_device == "Motor":
    omega = np.asarray(device_response.get("derived", {}).get("motor_speed", np.zeros_like(time_array)))
    omega_limit = device_response.get("state", {}).get("omega_limit", 1.0)

    tail = max(1, len(omega)//10)
    avg_omega = float(np.mean(omega[-tail:]))

    speed_ratio = np.clip(avg_omega / max(omega_limit, 1e-6), 0.0, 1.0)

    motor_animation_speed = 3 - speed_ratio * 2.4
    motor_animation_speed = np.clip(motor_animation_speed, 0.4, 3.0)
else:
    motor_animation_speed = 2.0
  


# 🔽🔽 ADD YOUR BLOCK RIGHT HERE 🔽🔽

# LED intensity based on actual current
if selected_device == "LED":
    led_current = np.asarray(device_response.get("current", np.zeros_like(time_array)))
    tail = max(1, len(led_current)//10)
    avg_current = float(np.mean(led_current[-tail:]))

    led_intensity = np.clip(avg_current / 0.02, 0.1, 1.0)
else:
    led_intensity = 0.2

# Buzzer speed based on envelope
if selected_device == "Buzzer":
    envelope = np.asarray(device_response.get("primary", np.zeros_like(time_array)))
    tail = max(1, len(envelope)//10)
    avg_env = float(np.mean(envelope[-tail:]))

    buzzer_pulse_speed = 1.8 - avg_env * 1.2
    buzzer_pulse_speed = np.clip(buzzer_pulse_speed, 0.4, 2.0)
else:
    buzzer_pulse_speed = 1.2
...
with col2:
    ...
    if selected_device == "Buzzer":
        ...
        buzzer_pulse_speed = np.clip(1.8 - avg_env * 1.2, 0.4, 2.0)
    else:
        buzzer_pulse_speed = 1.2
    ...
    if selected_device == "LED":
        ...
        st.markdown(f"""
        <div class="feature-card">
            <div class="feature-title">Heat Output</div>
            <div class="heat-stage">
                <div class="heat-bars">
                    <span style="height:{heat_bar_height}px;"></span>
                    <span style="height:{heat_bar_height + 6}px;"></span>
                    <span style="height:{heat_bar_height + 10}px;"></span>
                </div>
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    if device == "LED":
        ...
        for i in range(len(vin)):
            ...
            if v_d <= vf:
                ...
                diode_i = isat * (np.exp((v_d - vf) / (ideality * vt)) - 1.0)
                led_current[i] = diode_i / (1.0 + diode_i * r_led / max(v_d, 1e-6))
